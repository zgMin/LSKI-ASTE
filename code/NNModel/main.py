...
def train(args):
    ...
    for i in range(args.epochs):
        ...
        for j in trange(trainset.batch_count):
            _, sentence_tokens, lengths, masks, aspect_tags, _, tags , pos_ids, dep_ids= trainset.get_batch(j)
            predictions = model(sentence_tokens, lengths, masks,pos_ids, dep_ids)
...
def eval(model, dataset, args):
    ...
    with torch.no_grad():
        ...
        for i in range(dataset.batch_count):
            sentence_ids, sentence_tokens, lengths, mask, aspect_tags, _, tags,  pos_ids, dep_ids = dataset.get_batch(i)
            prediction = model.forward(sentence_tokens,lengths, mask,pos_ids, dep_ids)
...
def test(args):
    ...
    model.fusion.device = args.device
    model.eval()
    word2index = json.load(open(args.prefix + 'doubleembedding/word_idx.json'))
