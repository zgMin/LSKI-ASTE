...
def train(args):
    ...
    for i in range(args.epochs):
        ...
        for j in trange(trainset.batch_count):
            _, tokens, lengths, masks, _, _, aspect_tags, tags, pos_ids, dep_ids = trainset.get_batch(j)
            preds = model(tokens, masks, pos_ids, dep_ids)
...
def eval(model, dataset, args):
    ...
    with torch.no_grad():
        ...
        for i in range(dataset.batch_count):
            sentence_ids, tokens, lengths, masks, sens_lens, token_ranges, aspect_tags, tags, pos_ids, dep_ids = dataset.get_batch(i)
            preds = model(tokens, masks, pos_ids, dep_ids)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prefix', type=str, default="../../data/ASTE_DATA_V2/",
    ...
    parser.add_argument('--device', type=str, default="cuda",
                        help='gpu or cpu')
    parser.add_argument('--is_ernie', type=int, default=0,
                        help='0 or 1')
    parser.add_argument('--ablation_learner', type=int, default=0,
                        help='0 or 1')
    parser.add_argument('--ablation_fusion', type=int, default=0,
                        help='0 or 1')
    ...
    if args.task == 'triplet':
        args.class_num = 6
    if args.is_ernie:
        args.bert_model_path = "nghuyong/ernie-2.0-base-en"
        args.bert_tokenizer_path = "nghuyong/ernie-2.0-base-en"
    if args.mode == 'train':
        train(args)
        test(args)
    else:
